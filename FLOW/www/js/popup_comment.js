...
function block_response(response_data, response_is_specifique) {
	...
	if (response_data.Comment) {
		// CTRL+F hack_response pour les explications
	...
	if (you_have_to_prepend_response_specifique || response_is_specifique) {
		$(current_comment_block.fblock_response_container)
			.find(">:first-child")
			.after(this.fblock_response);
	} else {
		$(current_comment_block.fblock_response_container).append(
			this.fblock_response
		);
	}
	...
	$(this.f_response_img_user).css(
		"background-image",
		"url(" + response_data.ProfilePicture + ""
	);
	...
	this.f_response_date.innerHTML =
		response_data.Time == "0" ? "1 min" : set_timestamp(response_data.Time);
	...
	if (connected) {
		this.fresponse_like.src =
			this.is_liked == 0 ? "src/icons/Like.png" : "src/icons/Like_filled.png";
	} else {
...
function block_comment(comment_data, comment_is_specifique) {
	...
	$(this.fimg_user).css(
		"background-image",
		"url(" + comment_data.ProfilePicture + ""
	);
	...
	this.fblock_comment_comment.innerHTML =
		this.fcomment_text.replace(
			/@[^ ]+/gi,
	...
	this.fdate.innerHTML =
		comment_data.Time == "0" ? "1 min" : set_timestamp(comment_data.Time);
	$(this.fblock_comment_comment).append(this.fdate);

	this.fblock_comment_label_repondre = document.createElement("label");
	this.fblock_comment_label_repondre.className =
		"fblock_comment_label_repondre";
	...
	if (this.nombre_de_reponses > 0) {
		this.fblock_comment_label_afficher_les_reponses = document.createElement(
			"label"
		);
		this.fblock_comment_label_afficher_les_reponses.className =
			"fblock_comment_label_afficher_les_reponses";
		this.fblock_comment_label_afficher_les_reponses.innerHTML =
			"Afficher les reponses (" + this.nombre_de_reponses + ")";
		$(this.fblock_comment_comment).append(
			this.fblock_comment_label_afficher_les_reponses
		);
		...
		$(this.fblock_comment_label_afficher_les_reponses).on("click", function () {
			...
			if (current_comment_block.was_hidden == true) {
				...
				$(current_comment_block.fblock_response_container).css(
					"height",
					"" + current_comment_block.response_container_previous_height + "px"
				);
				$(current_comment_block.afficher_plus_de_reponses_container).css(
					"display",
					"inline-flex"
				);
			} else {
	...
	this.fblock_comment_label_reponses_precedentes = document.createElement(
		"label"
	);
	this.fblock_comment_label_reponses_precedentes.className =
		"fblock_comment_label_reponses_precedentes";
	this.fblock_comment_label_reponses_precedentes.innerHTML =
		"Réponses précédentes (" + this.nombre_de_reponses + ")";
	$(this.fblock_response_container).append(
		this.fblock_comment_label_reponses_precedentes
	);
	...
	$(this.fblock_comment_label_reponses_precedentes).on("click", function () {
		...
		$("#popup-comment").append(loading_tl);
		lastScrollHeight =
			current_comment_block.fblock_response_container.scrollHeight;
		let data = {
			ObjectId: current_comment_block.ObjectId,
	...
	this.afficher_plus_de_reponses_container.className =
		"afficher_plus_de_reponses_container";
	...
	this.label_afficher_plus_de_reponses.className =
		"label_afficher_plus_de_reponses";
	this.label_afficher_plus_de_reponses.innerHTML =
		"Afficher plus (" + this.nombre_de_reponses + ")";
	$(this.afficher_plus_de_reponses_container).append(
		this.label_afficher_plus_de_reponses
	);
	...
	this.label_hide_and_up_arrow_grey_container.className =
		"label_hide_and_up_arrow_grey_container";
	$(this.afficher_plus_de_reponses_container).append(
		this.label_hide_and_up_arrow_grey_container
	);

	$(this.label_hide_and_up_arrow_grey_container).on("click", function () {
		current_comment_block = block_comment;
		current_comment_block.response_container_previous_height = $(
			current_comment_block.fblock_response_container
		).height();
		...
		scroll_to =
			$(".fblock_comment_content").scrollTop() +
			scroll_to -
			current_comment_block.response_container_previous_height / 2;
	...
	$(this.label_hide_and_up_arrow_grey_container).append(
		this.label_cacher_reponse
	);
	...
	if (typeof response != "string") {
		for (let i = 0; i < response.Data.length; i++) {
			...
			if (new_block_response.ObjectId == id_response_specifique) {
				$(new_block_response.fblock_response).css(
					"background-color",
					"#1A84EF26"
				);
		...
		if (single_block_response_to_scroll_to) {
			let single_block_response_position = $(
				single_block_response_to_scroll_to.fblock_response
			).position();
			$(".fblock_comment_content").scrollTop(
				single_block_response_position.top
			);
		}
		if (you_have_to_prepend_response_specifique == false) {
			// reponses suivantes
				...
				for (
					...
					if (
						tableau_comment_to_tag_users[i] ==
							all_tagged_users[i_all_tag].private_Id &&
...
function send_response_to_server(data) {
    ...
    */
		...
		for (let i = 0; i < tableau_response_to_tag_users.length; i++) {
			if (tableau_response_to_tag_users[i].slice(0, 1) == "@") {
				for (
					...
					if (
						tableau_response_to_tag_users[i] ==
							all_tagged_users[i_all_tag].private_Id &&
						registrationId != all_tagged_users[i_all_tag].RegisterId
					) {
	...
	$(current_comment_block.label_afficher_plus_de_reponses).text(
		"Afficher plus (" + current_comment_block.nombre_de_reponses + ")"
	);
	$(current_comment_block.fblock_comment_label_afficher_les_reponses).css(
		"opacity",
		"0"
...
$("#finput_comment").keyup(function () {
	...
	if (string_input_comment_split[split_lenght - 1].slice(0, 1) == "@") {
		...
		if (string_input_comment_split[split_lenght - 1] == "@") {
			...
			string_input_comment_split[split_lenght - 1].length > 1 &&
			string_input_comment_split[split_lenght - 1] != "@"
		) {
			for (let i = 0; i < 1; i++) {
				let data_user_search = {
					Index: IdentificationListCurrentIndex,
					Search: string_input_comment_split[split_lenght - 1].slice(
						1,
						string_input_comment_split[split_lenght - 1].length
					),
					Nb: 10,
				};
...
function color_like(block, like) {
	...
	if (block.fblock_response) {
		...
		// si c'est un like de commentaire
		if (like) {
			console.log("chris color is like like like ");
